...
class Critic(torch.nn.Module):
    ...
    def forward(self, first_law_sample, second_law_sample):
      ...
      return self.linear_relu_stack(first_law_sample), self.linear_relu_stack(second_law_sample)
...
def sampler1(length_batch, device):
  sample = torch.normal(mean=torch.zeros(length_batch), std=torch.ones(length_batch)).to(device).reshape(length_batch,1)
  return sample

def sampler2(length_batch,device):
  sample = torch.normal(mean=-10*torch.ones(length_batch), std=torch.ones(length_batch)).to(device).reshape(length_batch,1)
  return sample
...
if torch.cuda.is_available() :
  model.to(device)
  histo_distance = critic_training(sampler1, sampler2, model, optimizer, n_critic, device)
